        ...
        <tbody>
            @forelse ($dadosRelatorio as $chaveLocal => $registros)
                @php
                    ...
                    $razaoSocial = $dadosManuais[$chaveLocal]['razao_social'] ?? '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                    $cnpj = $dadosManuais[$chaveLocal]['cnpj'] ?? '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
                @endphp
                <tr class="group-header">
                    <td colspan="3">{{ $nomeLocal }} - {{ $codigoLocal }} | {!! $cnpj !!} - {!! $razaoSocial !!} </td>
                </tr>
                @foreach($registros as $linha)
                    <tr>
                        <td>{{ $linha->nome_colaborador }}</td>
                        <td class="uppercase">{{ $linha->data }}</td>
                        <td class="uppercase">{{ $linha->horas_totais }}</td>
                    </tr>
                @endforeach
            @empty
                <tr>
                    <td colspan="3" class="text-center">Nenhum registro encontrado para o período selecionado.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
